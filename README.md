...
- CLI commands for writing notes, todos, calendar events, event flows, images, and data cards
- Data cards can be generated by external AI / CLI agents as ECharts option JSON and written through the CLI without configuring an AI API key inside Canvas Workbench
...
canvas-workbench dashboard add "Sales data card" --option ./sales-option.json --prompt "Generate sales trend and channel share charts from virtual data" --generated-by claude-code --grid "AI Zone"
cat ./sales-option.json | canvas-workbench dashboard add "Sales data card" --stdin --grid "AI Zone"
...
### Data cards and natural-language generation

The recommended data-card workflow is: natural-language request → external AI/agent → ECharts option JSON → `canvas-workbench dashboard add` → Canvas Workbench data card.

```text
natural-language request -> external AI/agent creates ECharts option JSON -> canvas-workbench dashboard add -> data card
```

Canvas Workbench does not include built-in AI calls and does not require an AI API key for this workflow. Claude Code, ChatGPT, Gemini, Cursor, Codex, or another external agent can understand the request, invent or transform data, choose chart types, and produce JSON-only ECharts options; Canvas Workbench receives the JSON, stores it locally, and renders it.

Data-card generation requirements:

- Output a valid ECharts `option` JSON object.
- The JSON must be serializable. Do not include JavaScript functions, formatter callbacks, event handlers, or runtime code.
- If `series` exists, it must be an array.
- Keep option files under 512 KiB when possible.
- Prefer `backgroundColor: "transparent"` and colors that work on a dark canvas.
- For multiple data cards, vary chart types when useful: line/bar combo, donut, radar, heatmap, funnel, scatter, or gauge.
- Use `--prompt` to preserve the user's natural-language request.
- Use `--generated-by` to record the producing agent or CLI.

Example CLI command:

```bash
canvas-workbench dashboard add "Sales trend combo" \
  --option ./sales-trend-option.json \
  --prompt "Generate a combo chart from virtual weekly sales and order data" \
...
- CLI-created note/todo/calendar/image/data cards default to the canvas center.
...
Use this Skill when an agent needs to add notes, todos, calendar events, event flows, images, or AI-generated ECharts data cards into Canvas Workbench. The packaged Skill documents the natural-language → external AI/agent → JSON-only ECharts option → `canvas-workbench dashboard add` workflow. The Local API must be online before the Skill/CLI can write data.
