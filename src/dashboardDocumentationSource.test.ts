...
test('Chinese README documents natural-language AI to ECharts dashboard workflow', () => {
  assert.match(readmeZh, /自然语言需求 → 外部 AI \/ 智能体 → ECharts option JSON → canvas-workbench dashboard add → 数据卡片/)
...
test('CLI skill documents dashboard natural-language generation requirements', () => {
  assert.match(cliSkill, /Use data cards for AI-generated data visualizations/)
